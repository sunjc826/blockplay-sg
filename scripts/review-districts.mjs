// Local authored-scene screenshots. Start Vite and Chrome CDP9223 first.
// Usage: node scripts/review-districts.mjs [district-id ...]
...
const output = '.cache/district-review';
...
try {
  ...
  for(const id of ids) for(const view of ['overview', 'street']) {
    const detail = await evaluate(`window.review.render(${JSON.stringify(id)}, ${JSON.stringify(view)})`);
    const shot = await page.send('Page.captureScreenshot', { format: 'png', fromSurface: true });
    const file = `${id}-${view}.png`; await writeFile(`${output}/${file}`, Buffer.from(shot.data, 'base64'));

...
export default function FpsGame({ region = 'marina-bay', suspended = false, profile, onReward, onElimination, onConsume, onOpenShop, arena, onLeaveArena }: { region?: WorldZoneId; suspended?: boolean; profile: ArmoryProfile; onReward: (result: ExerciseReward) => void; onElimination: (id: string) => void; onConsume: (id: string) => void; onOpenShop: () => void; arena?: ArenaOptions; onLeaveArena?: () => void }) {
  ...
  return <div className={`fps-game ${isArena ? 'is-arena' : ''} ${fullscreen.immersive ? 'is-immersive' : ''}`} ref={stage} data-touch={touch ? 'on' : undefined} data-map-zone={region} data-phase={hud.phase} data-pilot={hud.pilotEnabled || undefined} data-pointer-locked={hud.locked} data-health={hud.health.toFixed(1)} data-armor={hud.armor.toFixed(1)} data-vehicle={hud.vehicle} data-vehicle-health={hud.vehicleHealth} data-vehicle-ammo={hud.vehicleAmmo} data-vehicle-damage={hud.vehicleDamage} data-speed={hud.vehicleSpeed.toFixed(2)} data-altitude={hud.altitude.toFixed(2)} data-player-x={hud.x.toFixed(3)} data-player-z={hud.z.toFixed(3)} data-player-yaw={hud.yaw.toFixed(4)} data-player-pitch={hud.pitch.toFixed(4)} data-arena={isArena || undefined} data-arena-connected={isArena ? hud.arenaConnected : undefined} data-arena-alive={hud.arenaSelf?.alive} data-arena-kills={hud.arenaSelf?.kills}>
    <div className="viewport fps-viewport">
      ...
      {isArena && playing && <><div className="arena-kill-feed" aria-label="Elimination feed">{hud.arena?.feed.slice(-4).map(event => <p key={event.id} className={event.killerId === arenaOptions.session.id ? 'own-kill' : event.victimId === arenaOptions.session.id ? 'own-death' : ''}>{event.text}</p>)}</div>{hud.arenaSelf && !hud.arenaSelf.alive && <div className="arena-respawn" role="status"><span>ELIMINATED</span><strong>Back in {Math.ceil(hud.arenaSelf.respawnIn)}s</strong><small>Infantry armor/ammo replenish on respawn · Vehicle ammo resets with the match.</small></div>}{!hud.arenaStarted && !disconnected && <div className="arena-waiting" role="status">Waiting for the host to start the match.</div>}</>}
      <FpsRadio hud={hud} />
      {hud.pilotEnabled && playing && <div className="fps-pilot-indicator">AI PILOT · {hud.pilotStatus} · ESC TO STOP</div>}
      {playing && <>
        <FpsMinimap zone={region} player={hud} markers={hud.mapMarkers} mode={isArena ? 'arena' : 'practice'} />
        <FpsVitals hud={hud} />
        ...
        {mounted && <div className="fps-vehicle-hud" data-damage={hud.vehicleDamage}><span>{hud.vehicle === 'car' ? 'UTILITY 01' : 'FALCON 01'} / {hud.vehicleSeats[hud.vehicleSeat ?? 0]?.label.toUpperCase()}</span><strong>{Math.round(hud.vehicleSpeed)}<small>KM/H</small></strong>{hud.vehicle === 'helicopter' && <b>{hud.altitude.toFixed(1)} m ALT</b>}<div className="fps-vehicle-vitals"><label>HULL {Math.ceil(hud.vehicleHealth)} / {hud.vehicleMaxHealth} HP · {hud.vehicleDamage.toUpperCase()}<meter min={0} max={hud.vehicleMaxHealth} value={hud.vehicleHealth} /></label><b>{hud.vehicleWeapon} · {hud.vehicleAmmo} / {hud.vehicleAmmoMax}</b><small>{!hud.vehicleCanFire ? 'GUNNER CONTROLS WEAPON' : hud.vehicleAmmo ? 'LMB / FIRE · LOOK TO AIM' : 'AMMUNITION DEPLETED'}</small></div><div className="fps-vehicle-seats">{hud.vehicleSeats.map((seat, i) => <span key={seat.label} data-self={seat.self || undefined}>{i + 1}. {seat.label}: {seat.name}</span>)}</div><p>V · SWITCH SEAT · E · EXIT</p><p>{hud.vehicle === 'helicopter' ? 'SHIFT BOOST · LAND TO DISMOUNT' : 'BRAKE TO DISMOUNT'}</p></div>}
      </>}
      ...
      {!playing && !isArena && <div className="fps-overlay"><div className="fps-start-card">
        ...
        <div className="fps-control-guide"><span><kbd>WASD</kbd> Move</span><span><kbd>LMB</kbd> Fire</span><span><kbd>Q / RMB</kbd> Toggle / hold aim</span><span><kbd>R</kbd> Reload</span><span><kbd>Shift</kbd> Sprint</span><span><kbd>C</kbd> Crouch</span><span><kbd>Z</kbd> Prone / mount</span><span><kbd>Space</kbd> Jump</span><span><kbd>1–3</kbd> Switch</span><span><kbd>E</kbd> Enter / exit vehicle</span><span><kbd>Space / C</kbd> Fly up / down</span><span><kbd>F</kbd> Fullscreen</span></div>
    ...
    <FpsRadioVoice hud={hud} engine={engine.current} />
    <FpsPilotPanel hud={hud} engine={engine.current} suspended={suspended} />
    <div className="fps-loadout" aria-label="Weapon selection">{equipment.carriedFamilies.map((i, slot) => { const w = equipment.weapons[i]; return <button key={w.id} aria-pressed={i === hud.weapon} disabled={hud.phase === 'loading' || hud.phase === 'error'} onClick={() => engine.current?.switchWeapon(i)}><kbd>{slot + 1}</kbd><span>{w.name}<small>{w.role}</small></span><span className="fps-selected">{i === hud.weapon ? 'EQUIPPED' : 'EQUIP'}</span></button>; })}{hud.quickItem && <button className="fps-supply" data-testid="quick-item" disabled={!hud.quickCount || hud.phase !== 'playing'} onClick={() => engine.current?.useQuickItem()}><kbd>G</kbd><span>{hud.quickItem}<small>{hud.quickType}</small></span><span className="fps-selected">x{hud.quickCount}</span></button>}<div className="fps-accuracy"><span>ACCURACY</span><strong>{accuracy}%</strong></div></div>
    <div className="fps-inputs" aria-label="On-screen FPS controls">
      <div className="fps-dpad">{(['a', 'w', 's', 'd'] as const).map((key, i) => { const Icon = [ArrowLeft, ArrowUp, ArrowDown, ArrowRight][i]; return <button key={key} disabled={!canFight || (mounted && hud.vehicleSeat !== 0)} aria-label={`FPS ${['left', 'forward', 'backward', 'right'][i]}`} {...hold(key)}><Icon size={18} /></button>; })}</div>
      <button disabled={!canFight || (mounted && hud.vehicleSeat !== 0)} {...hold('shift')}>{hud.vehicle === 'helicopter' ? 'Boost' : 'Sprint'}</button><button disabled={!canFight || (mounted && hud.vehicleSeat !== 0)} {...hold('c')}>{hud.vehicle === 'helicopter' ? 'Descend' : 'Crouch'}</button>{mounted ? <button disabled={!canFight || hud.vehicleSeat !== 0} {...hold(' ')}>{hud.vehicle === 'car' ? 'Brake' : 'Climb'}</button> : <button disabled={!canFight} onClick={() => engine.current?.jump()}>Jump</button>}<button disabled={!playing || !hud.interact} onClick={() => engine.current?.interactVehicle()}>{mounted ? 'Exit vehicle' : 'Enter vehicle'}</button>{mounted && <button disabled={!canFight} onClick={() => engine.current?.switchVehicleSeat()}>Switch seat</button>}<button disabled={!canFight || mounted} onClick={() => engine.current?.setInput('z', true)}>{hud.prone ? 'Stand' : 'Prone'}</button><button disabled={!canFight || mounted} onClick={() => engine.current?.reload()}>Reload</button><button disabled={!canFight || mounted} title="Toggle aim (Q)" aria-pressed={hud.aiming} onClick={() => engine.current?.toggleAim()}>Aim</button><button className="fps-fire-button" disabled={!canFight || (mounted && (!hud.vehicleCanFire || hud.vehicleAmmo === 0))} {...hold('fire')}>Fire</button>
    </div>
    ...
    <p className="fps-message" role="status">{fullscreen.notice || hud.message || (isArena ? 'Match scores are session-only · Infantry armor/ammo replenish on respawn · Vehicle ammo resets with the match' : 'Game balance stats · Scenery blocks shots · Armor replenishes each exercise')}</p>
  </div>;
}

...
import { ArrowUpRight, Check, ChevronRight, Crosshair, Download, Globe2, MapPin, RefreshCw, Users, X } from 'lucide-react';
...
import { pwa, usePwa } from './lib/pwa';
  ...
  return <div className="app-shell">
    <header className="site-header">
      ...
      <div className="header-actions">
        {install.canInstall && <button className="install-button" onClick={() => { void pwa.install(); }}><Download size={15} /><span>Install</span></button>}
    ...
    <main>
      ...
      <section className="workspace" aria-label="Singapore playground">
        ...
        <div className="experience">
          {expeditionOpen ? <ExpeditionGame initialZone={expeditionStart} destination={destination} onZoneChange={zone => setLocation(locations.find(place => place.id === zone)!)} profile={armory.profile} onVendorPurchase={armory.buyFromVendor} onConsume={armory.consume} onExit={openFps} suspended={dialog !== null} /> : lobbyOpen ? <LanLobby onLaunch={launchArena} onBack={openFps} /> : armoryOpen ? <ArmoryShop store={armory} rangeLabel={getFpsDistrict(location.id).label} onEnterRange={openFps} /> : fpsOpen ? <FpsGame key={arena?.session.id || `practice-${location.id}`} region={location.id} arena={arena} onLeaveArena={openLobby} profile={armory.profile} onReward={armory.award} onElimination={armory.awardElimination} onConsume={armory.consume} onOpenShop={openArmory} suspended={dialog !== null} /> : regionOpen && hasRegionGame(location.id) ? (location.id === 'marina-bay' ? <MarinaGame key={location.id} /> : <RegionGame key={location.id} region={location.id} />) : <>
    ...
    {install.updateReady && <div className="update-toast" role="status"><span>A new version is ready.</span><button onClick={() => { pwa.applyUpdate(); }}><RefreshCw size={12} /> Reload</button></div>}

...
Implemented 38 authored routes (two per district) in parallel, prioritising existing bridge/connector conversions, housing galleries, industrial walkways and modest landscape terraces. See docs/VERTICALITY.md and four linked group evidence documents. This is a first playable elevation pass, not surveyed architecture; no new Google requests, reference acceptance changes or capture-plan changes. Low terraces initially resembled generic freestanding bridges; visual review changed ten to solid foundations.

New vertical-routes.ts shares rendered ramp/deck top planes with authored support surfaces. vertical-movement.ts handles support, small steps, falls/jumps, headroom, rails and solid foundations with spatial obstacle bins. FPS and both exploration components use it; arena actors/prone/cover preserve elevation. Bots receive physics but retain existing steering. Cars remain street-level; elevated walk→Drive returns to district spawn. Ground interaction prompts/actions and collectible stamps cannot activate from upper decks. Loot placement accepts a canStand callback and uses actual ground clearance.

Adjusted Bishan/Changi/Geylang sector bounds; moved obstructed ground anchors and Punggol's former channel stamp/encounter to the bank; updated six measured cover labels. Loot weights and area pricing preserved. All sector anchors, patrol spawns, and seeded loot placement checked against actual standing clearance.

...
Targets alternate 100 and 115 health. Vanguard clears a 115-health target in three landed shots versus four for the issued SAR. The dossier draws two charts: damage against range, over hits-to-kill bands, and sight climb against rounds held, against the width of a target at 20 m. Both simulate through the engine's own code — `hitDamage` and the recoil module — so the shop cannot advertise a figure the range does not produce. Attachments modify reload, capacity, recoil, recoil recovery, aiming FOV or movement. The quick-change and extended magazines share one slot, so one replaces the other, and the three foregrips share the handling slot on a strict ladder — recoil is what that slot sells. Unlocks fit both weapons, with equipment saved separately for each platform. Skin palettes change only materials. Variant accents, the hardware a variant is built from and equipped attachment markers all appear in the preview and viewmodel.
